...
import { Loader2, MessageSquare, ShieldAlert, Trash2 } from "lucide-react";
...
interface ThreadViewProps {
  ...
  onBack?: () => void;
  /** Kept for API compatibility; not read by this component */
...
type ThreadAttachment = { type?: string; url?: string };
...
export default function ThreadView({
  ...
  if (error || !thread) {
    return (
      <div className="text-center py-12">
        <p className="text-[var(--color-danger-main)] mb-4">
  ...
  const isThreadOwner = !!currentUserId && currentUserId === thread.user_id;
  const canDeleteThread = isThreadOwner || isSuperAdmin;
  ...
  const handleDeleteThread = async () => {
    try {
      ...
      toast.success(
        isSuperAdmin && !isThreadOwner
          ? "Discussion removed"
          : "Discussion deleted",
      );
      setShowDeleteThread(false);

      // Defer navigation so Radix Dialog unmount/restore-focus does not block
      // Next.js client navigation (especially when onBack is omitted on /explore).
      const leaveThread = () => {
        if (onBack) {
          onBack();
        } else {
      ...
      setTimeout(leaveThread, 0);
    } catch (e) {
      toast.error(
        e instanceof Error ? e.message : "Failed to delete discussion",
      );
    } finally {
  ...
  return (
    <div className="bg-white min-h-screen font-[var(--font-bell)] pb-32 p-4 rounded-lg">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6 shadow-sm">
        ...
        <div className="flex justify-between items-start gap-2 mb-4">
          ...
          {canDeleteThread && (
            <button
              ...
              className="shrink-0 p-2 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 border border-transparent hover:border-red-100 transition-colors inline-flex items-center gap-1"
              title={
                isSuperAdmin && !isThreadOwner
                  ? "Delete discussion (Super Admin)"
                  : "Delete discussion"
              }
            >
        ...
        {Array.isArray(thread.attachments) && thread.attachments.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            {(thread.attachments as ThreadAttachment[]).map((att, i) => (
      ...
      <ConfirmDeleteDialog
        open={showDeleteThread}
        onOpenChange={setShowDeleteThread}
        ...
        description={
          isSuperAdmin && !isThreadOwner
            ? "You are removing this discussion as Super Admin. All replies will be permanently deleted."
            : "This will permanently delete the discussion and all replies. This cannot be undone."
        }
        confirmLabel={isDeletingThread ? "Deleting…" : "Delete"}
