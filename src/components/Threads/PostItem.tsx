...
export default function PostItem({
  ...
  const handleChildEdit = (childId: number, content: string) => {
    setLocalReplies((prev) =>
      prev.map((r) => (r.id === childId ? { ...r, content } : r)),
    );
  ...
  const contentSection: any = isEditing ? (
    ...
    <p className="text-gray-700 text-sm mb-2 whitespace-pre-wrap leading-relaxed font-[var(--font-bell)]">
        ...
        {showReplyForm && (
          <div className="mt-3 ml-3">
            <CommentEditor
              ...
              placeholder={
                isAuthenticated ? "Write your reply..." : "Sign in to reply"
              }
            />
          </div>
        )}
