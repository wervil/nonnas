...
export const DashboardRecipe = ({
  ...
  if (loading)
    return (
      <div className="p-4 flex flex-row gap-1 w-full h-[100vh] items-center justify-center">
        <LoaderCircle className="animate-spin" /> {b("loading")}
      </div>
    );
  ...
  return (
    <div className="w-full mx-auto">
      <div className="flex gap-4 p-4 action-buttons">
        <Button onClick={goBack} className="sm:block hidden">
          {b("goBack")}
        </Button>
        <Button onClick={goBack} className="block sm:hidden">
        ...
        {hasAdminAccess ? (
          <>
            ...
            <Button className="sm:block hidden" onClick={() => window.print()}>
              {b("browserPrint")}
            </Button>
            <Button
        ...
        <Button
          className="block sm:hidden !px-3 bg-red-400"
          onClick={() => setShowDeleteDialog(true)}
          disabled={deleting}
        >
          {deleting ? (
