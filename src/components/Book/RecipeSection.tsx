...
// Videos are now uploaded separately ("Video of Nonna in the kitchen") and
// surfaced via a dedicated "Watch Video" button on the first page of the
// entry — the recipe view stays photo-only.
...
export const RecipeSection = ({
  ...
  // Defensive: never render legacy video URLs on the recipe page.
  const photoImages = (images ?? []).filter((u) => !isVideoUrl(u))
    ...
    <>
      ...
      {isOpen
        ? createPortal(
          <div
          onClick={closeModal}
          className="fixed mx-auto inset-0 z-1000 flex items-start md:items-center justify-center bg-[rgba(0,0,0,0.8)] p-4 overflow-y-auto">
            <button
              onClick={closeModal}
              title="Close"
            ...
            <div
              ...
              <div className="relative overflow-y-auto max-h-[calc(100vh-5rem)]">
                ...
                <h4 className="text-federant text-brown-light text-center text-m xl:text-xl">
                  {title}
                </h4>
