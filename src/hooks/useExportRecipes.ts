...
export const useExportRecipes = () => {
  ...
  const exportRecipesToZip = async (
    recipes: Recipe[],
    filterName: string = "Export",
  ) => {
    ...
    try {
      ...
      const promises = recipes.map(async (recipe, index) => {
        const grandmotherName =
          `${recipe.grandmotherTitle} ${recipe.firstName} ${recipe.lastName}`.trim();
        const folderName =
          `${recipe.country} - ${grandmotherName} - ${recipe.recipeTitle}`.replace(
            /[\/\\?%*:|"<>]/g,
            "-",
...
${stripHtml(recipe.directions)}
        ...
        if (recipe.photo && recipe.photo.length > 0) {
          ...
          await Promise.all(
            recipe.photo.map((url, i) =>
              addImageToFolder(url, nonnaFolder, "Nonna", i),
            ),
          );
        }
        ...
        if (recipeImages.length > 0) {
          ...
          await Promise.all(
            recipeImages.map((url, i) =>
              addImageToFolder(url, recipeImgFolder, "Recipe", i),
            ),
          );
        }
      });
