import { likes, threads, type NewThread } from "@/db/schema";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const scope = searchParams.get("scope"); // 'country', 'state', or 'city'
    ...
    // Build hierarchical filters based on geographic hierarchy
    if (region && scope) {
      if (scope === "country") {
        // For country scope: show country threads + all state/city threads in this country
        filters.push(
          sql`(${threads.scope} = 'country' AND ${threads.country} = ${region}) OR 
               (${threads.scope} = 'state' AND ${threads.country} = ${region}) OR
               (${threads.scope} = 'city' AND ${threads.country} = ${region})`,
        );
      } else if (scope === "state") {
        // For state scope: show state threads + all city threads in this state
        filters.push(
          sql`(${threads.scope} = 'state' AND ${threads.state} = ${region}) OR 
               (${threads.scope} = 'city' AND ${threads.state} = ${region})`,
        );
      } else if (scope === "city") {
        // For city scope: only show city threads for this city
        filters.push(eq(threads.city, region));
        filters.push(eq(threads.scope, "city"));
      }
    } else if (region) {
      // If no scope specified, just filter by region (for backward compatibility)
    ...
    if (sort === "top") {
      ...
      query.orderBy(
        sql`(${count(likes.id)} * 5) + (${threads.view_count} * 1) DESC`,
      );
    } else {
      // Default: Newest
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { region, scope, country, state, city, title, content, attachments } =
      body;
    ...
    if (scope !== "country" && scope !== "state" && scope !== "city") {
      return NextResponse.json(
        { error: 'Invalid scope. Must be "country", "state", or "city"' },
        { status: 400 },
      );
    }
