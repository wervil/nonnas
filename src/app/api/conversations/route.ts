...
export async function GET() {
  ...
  try {
    ...
    for (const convo of userConversations) {
      // Check user 1
      if (
        convo.user1_id !== user.id &&
        (!convo.user1_name || convo.user1_name === convo.user1_id)
      ) {
        missingUserIds.add(convo.user1_id);
      }
      // Check user 2
      if (
        convo.user2_id !== user.id &&
        (!convo.user2_name || convo.user2_name === convo.user2_id)
      ) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [u1, u2] = isUser1CurrentUser
    ...
    const existing = await db
      ...
      .from(conversations)
      .where(
        and(eq(conversations.user1_id, u1), eq(conversations.user2_id, u2)),
      )
      .limit(1);
    ...
    if (existing.length > 0) {
      ...
      if (
        u1Name &&
        u1Name !== u1 &&
        (!convo.user1_name || convo.user1_name === u1)
      ) {
      ...
      if (
        u2Name &&
        u2Name !== u2 &&
        (!convo.user2_name || convo.user2_name === u2)
      ) {
