...
function buildClusterLayers(rows: GroupRow[]) {
  ...
  rows.forEach((r, i) => {
    ...
    const stateCoords = count === 1 && parseCoordinates(r.coordinates)
      ? useCoords
      : regionCoords;
...
async function fetchIndividualMarkers(
  ...
  if (regionFilter) {
    conditions.push(eq(recipes.region, regionFilter));
  }
  if (cityFilter) {
    conditions.push(eq(recipes.city, cityFilter));
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (level === "CITY") {
      ...
      let clusters = cities;
      if (regionParam) {
        clusters = clusters.filter((c) => c.region === regionParam);
      }
      return NextResponse.json(
        { clusters },
        { headers: { "Cache-Control": "no-store" } },
