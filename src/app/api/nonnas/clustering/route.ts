...
import { getCountryInfoWithFallback, getRegionCoordinates } from "@/lib/countryData";
    ...
    try {
        ...
        if (level === "ALL") {
            ...
            allRows.forEach((r, i) => {
                ...
                // State/region layer
                const regionCoords = getRegionCoordinates(regionName, countryInfo.code, countryInfo.lat, countryInfo.lng);
                stateClusters.push({
                    id: `region-${i}`,
        ...
        if (level === "EARTH") {
            ...
            const conditions = [];
            if (countryParam) {
                conditions.push(eq(sql`lower(${recipes.country})`, countryParam.toLowerCase()));
            } else {
            ...
            clusters = rows.map((r) => {
                ...
                const regionCoords = getRegionCoordinates(r.region, countryInfo.code, countryInfo.lat, countryInfo.lng);
                return {
                    id: `nonna-${r.id}`,
                    lat: regionCoords.lat + randLat,
                    lng: regionCoords.lng + randLng,
                    countryCode: countryInfo.code,
                    countryName: r.country,
                    nonnaCount: 1,
