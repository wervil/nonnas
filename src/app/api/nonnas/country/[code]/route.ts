...
function hashStringToOffset(str: string): {
  latOffset: number;
  lngOffset: number;
} {
  let hash = 0;
...
> = {
  US: {
    ...
    louisiana: { lat: 31.2448, lng: -92.145 },
  ...
  IT: {
    ...
    calabria: { lat: 38.906, lng: 16.5943 },
  ...
  PK: {
    ...
    "gilgit-baltistan": { lat: 35.8186, lng: 74.598 },
    "gilgit baltistan": { lat: 35.8186, lng: 74.598 },
    ...
    "federally administered tribal areas": { lat: 32.7167, lng: 70.0 },
    fata: { lat: 32.7167, lng: 70.0 },
  },
  MX: {
  ...
  FR: {
    "île-de-france": { lat: 48.8499, lng: 2.637 },
    provence: { lat: 43.9352, lng: 6.0679 },
    normandy: { lat: 49.1829, lng: -0.3707 },
    brittany: { lat: 48.202, lng: -2.9326 },
    bordeaux: { lat: 44.8378, lng: -0.5792 },
    lyon: { lat: 45.764, lng: 4.8357 },
  },
  VE: {
    bolívar: { lat: 7.0, lng: -63.5 },
    bolivar: { lat: 7.0, lng: -63.5 },
    caracas: { lat: 10.4806, lng: -66.9036 },
    zulia: { lat: 10.0, lng: -71.8 },
    miranda: { lat: 10.3, lng: -66.5 },
    aragua: { lat: 10.25, lng: -67.5 },
    carabobo: { lat: 10.2, lng: -68.0 },
    lara: { lat: 10.05, lng: -69.85 },
  },
  CO: {
  ...
  SE: {
    ...
    umeå: { lat: 63.8258, lng: 20.263 },
    gävle: { lat: 60.6749, lng: 17.1413 },
    borås: { lat: 57.721, lng: 12.9401 },
    ...
    for (const regionName of regionsList) {
      ...
      for (const [key, value] of Object.entries(stateCoords)) {
        if (normalizeRegionName(key) === normalizedRegion) {
          coords = value;
          console.log(
            `  ✓ Found hardcoded coords for "${regionName}" → (${value.lat}, ${value.lng})`,
          );
          break;
        }
      }
      ...
      if (!coords) {
        for (const [key, value] of Object.entries(stateCoords)) {
          ...
          if (
            normalizedKey.includes(normalizedRegion) ||
            normalizedRegion.includes(normalizedKey)
          ) {
            coords = value;
            console.log(
              `  ✓ Partial match hardcoded coords for "${regionName}" → "${key}" → (${value.lat}, ${value.lng})`,
            );
            break;
          }
        }
      ...
      if (!coords) {
        ...
        if (geoJsonCoords) {
          ...
          console.log(
            `  ✓ Found GeoJSON coords for "${regionName}" → (${coords.lat}, ${coords.lng})`,
          );
        }
      }
      ...
      if (!coords) {
        const offset = hashStringToOffset(
          `${code.toUpperCase()}-${regionName}`,
        );
        coords = {
        ...
        console.log(
          `  ⚠ Using hash-based fallback for "${regionName}" → (${coords.lat}, ${coords.lng})`,
        );
      }

      console.log(
        `  → Final coordinates for "${regionName}": (${coords.lat}, ${coords.lng})\n`,
      );
